...
export function VideoGrid({
  ...
  const tTooltipMore = isArabic ? "خيارات إضافية ومميزات البث ومعلومات الرابط" : "Extra options, stream features & connection info";
  ...
  return (
    <div className={`flex flex-col gap-4 h-full ${isRtl ? "text-right" : "text-left"}`}>
      ...
      <div className={`bg-[#110c28]/95 backdrop-blur-md border border-[#241f48] rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-[0_0_15px_rgba(108,71,255,0.05)] ${isRtl ? "flex-row-reverse" : "flex-row"}`}>
            ...
            <>
              ...
              <div className="absolute bottom-4 right-4 left-4 bg-[#110c28]/95 backdrop-blur-md border border-[#241f48] rounded-xl p-2.5 px-3.5 flex items-center justify-between z-10 shadow-lg">
                <span className="text-xs font-bold text-white">
                  {activeCall ? (activeCall.callerId === (localStream?.id || "local") ? lastRemoteName || t("remoteParticipantLabel") : activeCall.callerName) : lastRemoteName || t("remoteParticipantLabel")}
                </span>
              ...
              <div className="relative mb-5 flex items-center justify-center">
                ...
                {(() => {
                  const resolvedRemoteName = activeCall
                    ? (activeCall.callerId === (localStream?.id || "local")
